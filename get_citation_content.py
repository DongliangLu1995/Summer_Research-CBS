# -*- coding: utf-8 -*-
"""
Created on Tue July  31 10:18:46 2018
...
def citation_content_num(Org_citation, Org_citation_num):
    d={}
    for org_i in range(len(Org_citation)):
        org = Org_citation[org_i]
        if org in d:
            d[org].append(Org_citation_num[org_i])
        else:
            d[org] = [Org_citation_num[org_i]]
    for org in d.keys():
        d[org] = set(d[org])
    return d
...
for i_large in range(0,len(final_rule_good)):
    ...
    if Citation_total:
        citation_number=find_citation_num(Citation_total)
    else:
        citation_number=0
...
#        start_strip=re.match(start_strip_pattern,c).span()[1]
        strip_span = re.search(start_strip_pattern,c).span()
        start_strip = strip_span[1]
        num_start = strip_span[0]
        citation_num = c[num_start:start_strip].split()[0]
        c=c[start_strip:]
        cspan = re.search(start_pattern,c)
        while cspan:
            citation=c[: cspan.span()[0]]
            Citation.append(citation.strip())
            Citation_num.append(citation_num)
            citation_num = c[cspan.span()[0]:cspan.span()[1]].split()[0]
            c=c[cspan.span()[1]:]
            cspan = re.search(start_pattern,c)
    ...
    Citation_has_org=[]
    Citation_has_org_num=[]
    count=0
    for i in range(0,len(Citation)):
    #for i in range(0,200):
        citation_num = Citation_num[i]
        C=Citation[i]
        not_ind=0
        for n in not_org:
            if n in C:
        ...
        if not_ind==0:
            if ";" in C:
                ...
                Citation_has_org_num.append(citation_num)
            else:
                if "\n" not in C:
                    count_number=0
                    for w in C:
...
#        print(i)
        ...
        orgs_c_number=Citation_has_org_num[i]
        ...
        for orgs in orgs_c_l2:
            ...
            if orgs in org_set:
                ...
                Org_citation_num.append(orgs_c_number)
    ...
    org_citation_location = citation_content_num(Org_citation, Org_citation_num)
    ...
    location_save_pickle_add_i=r"C:\Users\DongliangLu\Desktop\Columbia\research\CBS\lobby\data_cleaned"+"\\SEC_citation_location_"+str(read_rule_num)+".pickle"
    pickle.dump(org_citation_location, open(location_save_pickle_add_i,"wb")) 
    
    citation_percent.append({})
    citation_percent[-1]["rule type number"]=read_rule_num
    citation_percent[-1]["total citation"]=citation_number
    ...
    citation_percent[-1]["citation stating details"]=citation_number-len(Citation)
