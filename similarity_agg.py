# -*- coding: utf-8 -*-
"""
Created on Sat Jul 21 12:01:07 2018
...
def cal_distance_matrix_new(comment_feature_array):
    r,c = comment_feature_array.shape
    col_sum = np.sum(comment_feature_array,axis=0)
    m = np.zeros(r)
    for row_i in range(r):
       t_factor = 1 - (col_sum - comment_feature_array[row_i]) / col_sum
       t_factor[np.isnan(t_factor)] = 0
       m[row_i] = (comment_feature_array[row_i]*t_factor).sum() / (comment_feature_array[row_i]).sum()
    return m
...
#    comments_df['pca tfidf similarity'] = 0
    ...
    comments_sep_dic_df['org'] = comments_df['identify org']
    comments_sep_dic_df['comment len'] = comments_df['comment len']
    
    comments_sep_dic_df_org = comments_sep_dic_df[ comments_sep_dic_df['org'] != 'Individual' ]
    comments_sep_dic_df_individual = comments_sep_dic_df[ comments_sep_dic_df['org'] == 'Individual' ]
    comments_sep_dic_df_org = comments_sep_dic_df_org.groupby(["org"]).sum()
    comments_sep_dic_df_org['identify org'] =  comments_sep_dic_df_org.index
    comments_sep_dic_df_org=comments_sep_dic_df_org.reset_index(drop=True)
    comments_sep_dic_df_individual = comments_sep_dic_df_individual.drop('org', 1)
    comments_sep_dic_df_individual['identify org'] = 'Individual'
    
#    similarity_matrix = cal_similarity_matrix(comments_sep_dic_df, "count")
    comments_sep_dic_df_new = comments_sep_dic_df_org.append(comments_sep_dic_df_individual)
    comments_sep_dic_df_new = comments_sep_dic_df_new.reset_index(drop=True)
    comment_len_sery =  comments_sep_dic_df_new['comment len'] 
    
    
    comments_sep_dic_df_append = comments_sep_dic_df_new.drop(['identify org','comment len'], 1)
    
    
    
    rule_corpus = merge_two_dic(comments_total_dic, final_rule_dic)
#    bag_comment_and_rule = comments_sep_dic+[final_rule_dic]
    final_rule_df = pd.DataFrame([final_rule_dic])
    bag_comment_and_rule_df_new = comments_sep_dic_df_append.append(final_rule_df)

#    bag_comment_and_rule_df = pd.DataFrame(bag_comment_and_rule)
    bag_comment_and_rule_df_new = bag_comment_and_rule_df_new.fillna(0)
    bag_comment_and_rule_array = bag_comment_and_rule_df_new.values
    ...
    distance_matrix_n = cal_distance_matrix_new(bag_comment_and_rule_array[:-1])
    distance_array_n = fill_nan_with_mean(distance_matrix_n)
    distance_matrix_set_n = cal_distance_matrix_new(bag_comment_and_rule_set_array[:-1])
    distance_array_set_n = fill_nan_with_mean(distance_matrix_set_n)
    distance_matrix_tfidf_n = cal_distance_matrix_new(bag_comment_and_rule_tfidf_array[:-1])
    distance_array_tfidf_n = fill_nan_with_mean(distance_matrix_tfidf_n)
...
#    pca_distance_matrix = cal_distance_matrix(bag_comment_and_rule_array_pca[:-1])
#    pca_distance_array = fill_nan_with_mean(pca_distance_matrix)
#    pca_distance_matrix_set = cal_distance_matrix(bag_comment_and_rule_set_array_pca[:-1])
#    pca_distance_array_set = fill_nan_with_mean(pca_distance_matrix_set)
#    pca_distance_matrix_tfidf = cal_distance_matrix(bag_comment_and_rule_tfidf_array_pca[:-1])
#    pca_distance_array_tfidf = fill_nan_with_mean(pca_distance_matrix_tfidf)




    comments_df_new = pd.DataFrame()
    comments_df_new['identify org'] = comments_sep_dic_df_new['identify org']
    comments_df_new['rule type'] = rule_type_i
    comments_df_new['comment len'] = comment_len_sery
    comments_df_new['count distance'] = distance_array
    comments_df_new['set distance'] = distance_array_set
    comments_df_new['tfidf distance'] = distance_array_tfidf
    comments_df_new['count distance n'] = distance_array_n
    comments_df_new['set distance n'] = distance_array_set_n
    comments_df_new['tfidf distance n'] = distance_array_tfidf_n    
    comments_df_new['pca count distance'] = pca_distance_array
    comments_df_new['pca set distance'] = pca_distance_array_set
    comments_df_new['pca tfidf distance'] = pca_distance_array_tfidf
...
#    comments_df['pca tfidf distance'] = pca_distance_array_tfidf
    ...
    for comment_i in range(len(comments_df_new)):
...
#    
    comments_df_new['count similarity'] = fill_nan_with_mean(np.array(count_similarity_array))
    comments_df_new['set similarity'] = fill_nan_with_mean(np.array(set_similarity_array))
    comments_df_new['tfidf similarity'] = fill_nan_with_mean(np.array(tfidf_similarity_array))
    comments_df_new['pca count similarity'] = fill_nan_with_mean(np.array(pca_count_similarity_array))
    comments_df_new['pca set similarity'] = fill_nan_with_mean(np.array(pca_set_similarity_array))
    comments_df_new['pca tfidf similarity'] = fill_nan_with_mean(np.array(pca_tfidf_similarity_array))
        
        
    
    
#    comments_df.fillna(0)
    comments_df_new.fillna(0)
    
    similarity_save_add = similarity_save_basic_add +"\\similarity_agg_"+"One"+"_" + str(rule_type_i) +".pickle"
    pickle.dump(comments_df_new, open(similarity_save_add,"wb"))
    similarity_csv_save_add = similarity_save_add.replace(".pickle",".csv")
    comments_df_new.to_csv(similarity_csv_save_add)
    
    count_distance_matrix_save_add = similarity_save_basic_add +"\\count_distance_matrix_agg"+"One"+"_" + str(rule_type_i) +".pickle"
    set_distance_matrix_save_add = similarity_save_basic_add +"\\set_distance_matrix_agg"+"One"+"_" + str(rule_type_i) +".pickle"
    tfidf_distance_matrix_save_add = similarity_save_basic_add +"\\tfidf_distance_matrix_agg"+"One"+"_" + str(rule_type_i) +".pickle"
    pca_count_distance_matrix_save_add = similarity_save_basic_add +"\\pca_count_distance_matrix_agg"+"One"+"_" + str(rule_type_i) +".pickle"
    pca_set_distance_matrix_save_add = similarity_save_basic_add +"\\pca_set_distance_matrix_agg"+"One"+"_" + str(rule_type_i) +".pickle"
    pca_tfidf_distance_matrix_save_add = similarity_save_basic_add +"\\pca_tfidf_distance_matrix_agg"+"One"+"_" + str(rule_type_i) +".pickle"
...
covariance = np.cov(comments_df_new[["count similarity","set similarity","tfidf similarity","count distance","set distance","tfidf distance"]].values, rowvar=False)
correlation = np.corrcoef(comments_df_new[["count similarity","set similarity","tfidf similarity","count distance","set distance","tfidf distance","count distance n","set distance n","tfidf distance n"]].values.T)
